...
def read_single_anndata(config, path=None):
    ...
    if getattr(config.data, "target") == "all":
        data.obs["transport"].fillna("target", inplace=True)

    mask = data.obs["transport"].notna()
    assert not hasattr(config.data, "subset")
    if config.datasplit.subset is not None:
    ...
    data = data[mask].copy()
    if hasattr(config, "datasplit"):
        data.obs["split"] = split_cell_data(data, **config.datasplit.to_dict() if config.datasplit else {})
...
def load_cell_data(
    ...
    **kwargs
    ...
    if "loader" in return_as:
        kwargs = config.dataloader.to_dict() if hasattr(config.dataloader, "to_dict") else config.dataloader
...
def split_cell_data_train_test(
    ...
    kwargs.pop("mode", None)  # Delete "mode" if it dosen't exist in kwargs
    kwargs.pop("key", None) 
